...
1. Read tracked entries with `nix eval --json '.#lib.rpiKernelSeries'` and confirm their overlay definitions.
...
5. Add `rpi-linux-X_Y-src` in `flake.nix` using `github:raspberrypi/linux/rpi-X.Y.y` and refresh only that lock input. The `lib.rpiKernelSeries` output and updater discover it from this input name.
6. Add the input argument and `vX_Y` entry in `overlays/default.nix` with `patches = [ ];`.
7. Confirm `--list-series` includes `vX_Y`, then run both board evaluations and ShellCheck.
8. Do not change `rpi/default.nix` or README defaults unless explicitly requested.
